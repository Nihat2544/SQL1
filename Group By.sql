...
GROUP BY genelde, AVG8(), COUN(), MAX(), MIN() VE SUM() gibi aggregate fonksiyonlari ile birlikte kullaninilir
*/
CREATE TABLE manav 
(
    isim varchar(50), 
...
max(urun_miktari)as maksimum_urun_mik from manav where urun_adi is not null 
group by urun_adi;
...
select ulke,cinsiyet, round (avg (yas)), count(*)  from personel where maas>30000 group by ulke, cinsiyet;
